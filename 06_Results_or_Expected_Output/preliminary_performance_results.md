...
The `performance_test.py` script repeatedly processes a representative synthetic IAM finding using the same rule-based mapping function implemented in `risk_mapper.py`.
...
Performance values may vary depending on hardware, operating system, Python version and other processes running on the test machine.